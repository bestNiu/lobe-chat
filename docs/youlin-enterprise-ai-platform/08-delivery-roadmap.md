...
> 状态：交付基线 2.4（已校正依赖切片、Pilot 验收和投入口径）
>
...
采用并行工作流，但身份、权限和审计必须先于企业能力开放。W26/W32 为保守排期；22 周建设或 4 周 Pilot 仅在不削减 P0 门禁、经容量重估批准后压缩。里程碑是最终验收点，不代表依赖能力只能届时开始；早期可消费切片见[依赖基线](./plan/04-unified-baseline-and-decision-register.md)。
...
- 身份/数据安全边界明确；仍在 Spike 的技术项按决策台账登记截止日和阻断的 Spec，不把 W3～W6 才能验证的技术选型虚标为 W2 已批准。
...
- PRD AC-28～AC-32 通过，AC-33 运营界面/Runbook 分项通过；完整恢复与发布演练在 M12 签收；
...
- PRD AC-01～AC-14、AC-16～AC-33 通过；AC-15 只检查 Pilot 准备，运行结果在 M14 验收；
- 无开放 P0/P1 缺陷、Critical/High 安全风险或硬性 No-Go；
- 备份、恢复、回滚和应急禁用演练通过；
...
- 不立即替代原知识和业务系统；
- 完成 AC-15 实际运行验收，形成 Stage 1/2 优先实施及 Stage 3 临床场景候选投入建议。
...
角色表各行区间相加为 **15.9～22.9 FTE**，不能直接解释为同周峰值。当前计划峰值 **15～19 FTE**，须通过分周错峰/兼职投入实现；若全角色同时满配应按 16～23 FTE 重估。建设投入约 **75～115 人月**，仅覆盖 22～26 周建设，Pilot 4～6 周人力另计；M0 先分配责任角色，W4 必须提交逐周容量/人月表，否则不是可承诺排期。统一首页/搜索、任务/通知、评审/反馈、企业配置和运营中心补齐了可用产品闭环；完整图谱、外部网关或复杂数据源适配仍需单独立项。
...
| 资源中心/OSS | C | A | R | C | R | I | C | R |
| 企业知识/记忆/产出物 | C | C | R | R | R | I | C | A/R |
| Project/Context/Agent 授权 | C | A | R | R | R | C | R | R |
| 首页/搜索/任务/评审/运营 | A/R | C | R | C | R | C | C | C |
| 湖仓/Data Product | C | R | R | I | R | C | C | A/R |
| API Product/授权 | C | R | R | I | R | C | R | A/R |
| 安全发布 | I | C | R | C | R | C | A/R | I |
| Pilot Go/No-Go | A | C | I | C | R | C | R | R |

每项只有一个最终负责 A；Security、Data/Knowledge/Project Owner 对其边界仍有强制签收权。上述角色必须在 M0 映射实名；Data/Knowledge Owner 列在项目事项由 Project Owner 承担，不意味着一人掌握所有领域授权。
...
| 环境、OSS、数据库、湖仓和监控 | 25～90 万 | 建设期 Dev/Test/UAT、Pilot 初始部署、对象版本、查询、Gateway 和备份；不含 Pilot 正式运行新增用量 |
...
表内不含预备金小计 **345～890 万元**，按 15%～20% 预备金计算为 **396.75～1,068 万元**，取整规划 ROM **约 400～1,100 万元**；人力与集成外包不得重复计费，Pilot 运行人力/用量另按实际方案增列。不包括完整企业知识图谱、全量历史项目/数仓迁移、大规模私有 GPU、外部生产 API 专区和 Office 多人实时协作套件。
...
| R27 | “后续对外”与当前数据不出境边界冲突 | 中 | 极高 | 外部数据产品逐个审批，跨境默认拒绝，湖仓专题阶段 D 独立 Go/No-Go | Privacy/Legal |
...
- PRD AC-01～AC-14、AC-16～AC-33 通过；AC-15 准备就绪，M14 才能签收实际 Pilot 结果；
...
- 无开放 P0/P1 缺陷或 Critical/High 安全风险；较低风险例外有批准人、补偿控制和到期日，不能豁免安全硬门禁；
