...
> 目标版本：MVP 1.0
>
> 目标周期：建设 22～26 周，Pilot 4～6 周
>
> 关联文档：[企业版二开架构](./04-lobehub-extension-architecture-and-roadmap.md) · [集成契约](./06-integration-contracts.md) · [多系统融合接入规范](./09-multi-system-fusion-integration-standard.md) · [湖仓与 API 治理](./10-lakehouse-data-platform-and-api-governance.md) · [记忆与上下文治理](./11-context-memory-and-agent-authorization-governance.md) · [完整产品蓝图](./12-full-product-capability-and-evolution-blueprint.md)
>
...
18. 补齐统一首页、导航、权限感知全局搜索、我的任务、通知、评审、反馈、Feature Flag 和管理员运营闭环。
...
提供权限感知的企业首页、统一导航、应用切换、最近会话、常用 Agent、我的项目、待办、通知和系统状态。全局搜索覆盖获授权的资源、知识、Project、Agent、应用和 Data Product；无权对象不得通过计数、补全或摘要泄漏。

### FR-K02 我的任务与通知

统一展示异步 Job、Workflow 人工节点、评审请求、失败重试和业务系统待办引用。通知支持站内、企业微信/邮件 Adapter、已读、订阅和免打扰。通知不构成业务终态，正式 OA 审批仍由泛微承担。

### FR-K03 统一评审中心

支持 Skill、Tool、Workflow、Agent 发布评审，知识发布/撤回，记忆 Promotion，以及数据/API 授权的提交、退回、批准、版本、SLA 和审计。高风险提交人不得审批自己。

### FR-K04 用户自助与反馈

用户可管理个人设置、通知、记忆开关、默认模型、Desktop 设备、资源/产出物、用量和授权；可对回答、引用、Agent、Workflow 和系统问题反馈，反馈具备分类、指派、状态、结果和通知。

### FR-K05 企业配置与 Feature Flag

企业管理员可配置品牌、公告、字典、默认策略、功能开关、灰度范围和紧急停用。Feature Flag 和 Entitlement 必须服务端执行、版本化并审计，不以隐藏前端按钮作为控制。
...
### 16.3 Agent、Skill、Workflow、RAG 与 Tool 分工
...
1. 登录与账号绑定；
2. 企业首页、统一导航、权限感知全局搜索和 Agent 工作台；
...
16. 统一评审中心、SLA 和职责分离；
17. Feature Flag、企业配置、品牌、公告和字典；
18. 审计、运行、队列、告警、成本、反馈和集成健康状态。
...
- 登录完成后首页 P95 ≤ 3 秒；
- 普通列表和权限感知全局搜索 P95 ≤ 2 秒（复杂全文检索阈值由压测冻结）；
- 资源列表 P95 ≤ 2 秒，普通文件上传后 95% 在 5 分钟内生成可用预览；
- 知识检索 P95 ≤ 5 秒，不含模型生成；
- 流式回答首 Token P95 ≤ 5 秒；
- 组织增量同步在 15 分钟内生效；
- 禁用用户访问回收时间由安全评估冻结，建议 ≤ 15 分钟；
- 任务/通知事件在正常依赖下 95% 于 1 分钟内形成可见投影，重复事件不重复投递；
...
异步任务、Workflow 人工节点、评审请求和失败重试可在统一任务中心查询；重要状态产生站内通知并可通过 Adapter 发送企业微信/邮件；重复事件不产生通知风暴，通知不被误认为审批终态。

### AC-30 统一评审中心

Capability 发布、知识发布、记忆 Promotion 和数据/API 授权均可提交、审核、退回、批准和追踪；高风险提交人不能批准自己，历史决定、版本、意见和 SLA 可审计。

### AC-31 用户自助和反馈

用户可管理通知、记忆、默认模型、Desktop 设备、资源/产出物、用量和授权；回答/引用/系统反馈可以提交、指派、处理并通知结果，支持人员能通过 Trace ID 定位链路。

### AC-32 企业配置和 Feature Flag

企业品牌、公告、字典、默认策略和功能开关可版本化配置、按环境/用户组灰度、审计和回滚；关闭高风险功能在服务端立即生效，不能只隐藏前端入口。

### AC-33 运营与恢复

管理员可以查看健康、队列、失败 Job、集成、配额、成本、质量和告警；上线前完成升级、回滚、备份、恢复、外部依赖降级和紧急停用演练，并具备管理员/Owner/用户/支持 Runbook。
...
| 工作台运营 | 首页任务到达率、搜索成功率、待办完成率、通知送达率、评审 SLA、反馈闭环率 |
| 稳定性 | P95 延迟、错误率、异步任务恢复率、回滚/恢复演练通过率 |
...
- 验证首页、全局搜索、任务、通知、评审、反馈、Feature Flag 和 Trace 报障闭环；
...
- 完整产品体按[产品能力与演进蓝图](./12-full-product-capability-and-evolution-blueprint.md)推进 Employee Workspace、Project Delivery OS、Clinical Copilot、Knowledge OS、AI Factory、Data/API Hub 和商业化能力。
...
26. 首页信息架构、全局搜索索引范围和排序；
27. 任务/通知/评审中心与泛微 OA、企业微信的职责边界；
28. Feature Flag、企业配置和 Entitlement 实现；
29. 反馈工单、Trace 报障和支持 SLA；
30. Stage 1～5 完整产品路线的业务 Owner、投资门和量化退出标准。
