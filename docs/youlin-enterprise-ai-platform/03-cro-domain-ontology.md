...
本文综合前两份蓝图形成候选模型，但尚未获得企业真实系统、制度文件和用户工作样本。因此：

- 本文中的“候选”表示推荐的初始定义，不表示企业已经采用；
- 源系统归属必须通过 02 的系统盘点和数据样本确认；
- 状态、动作和审批终态必须由业务负责人、质量与系统负责人共同签字确认；
- 受试者、安全病例等高敏感对象默认不进入通用 AI 上下文；
- 如本体与源系统冲突，先记录冲突，不直接覆盖源系统。
...
不要一次核验全文。先从 02 场景评分中选择一个首期闭环，例如“中心启动材料核验与审批”或“监查报告质检与行动项”。

输出：范围内的用户、对象、开始事件、终态事实、权威系统和基线。
