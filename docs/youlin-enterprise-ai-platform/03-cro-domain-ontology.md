...
| `Notification` | 通知 | 向接收人传达事实的消息 | 可订阅/去重；默认不产生审批终态 |
| `PlatformTask` | 平台任务 | 异步 Job、人工节点、评审或失败处置的统一任务投影 | 保留源任务和目标系统，不替代业务事实 |
| `ReviewRequest` | 评审请求 | Capability、知识、记忆 Promotion 或数据授权的治理决定请求 | 有提交人、Reviewer、SLA 和职责分离 |
| `UserFeedback` | 用户反馈 | 对回答、引用、能力或系统问题的结构化反馈 | 有分类、指派、状态、Trace 和结果 |
| `FeatureFlag` | 功能开关 | 按环境、租户、用户组控制能力启停和灰度的版本化配置 | 服务端强制执行和审计 |
| `SupportCase` | 支持工单 | 用户问题从报告到解决的可追踪记录 | 关联 Trace、Owner 和 SLA |
...
| --- | --- | --- | --- |
| 处理我的待办 | 等待本人决定、评审、重试或补充的事项 | 什么被我阻塞、风险、来源系统和时限 | PlatformTask、ReviewRequest、审批任务、升级 |
...
| 全局搜索 | 获授权的资源/项目/能力/应用/数据产品 | 最相关结果、范围、类型和权限 | Resource、Project、Agent、Module、DataProduct |
| 跟踪反馈 | 回答/引用/系统问题 | 谁处理、当前状态、何时完成 | UserFeedback、SupportCase、Trace |
