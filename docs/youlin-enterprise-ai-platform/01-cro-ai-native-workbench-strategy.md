...
以下内容不能从代码推断，必须在下一阶段调研：

- 公司组织架构、法人/事业部/部门/项目组关系；
- 现有 HR、OA、ERP、CRM、CTMS、EDC、eTMF、QMS、LMS、PV、安全数据库和财务系统；
- 每个系统的供应商、版本、接口、部署位置、数据责任人和合同限制；
- 真实用户任务、当前工时、返工率、等待时间、差错和稽查发现；
- 哪些电子记录属于 GxP/Part 11 或其他监管范围；
- 数据出境、跨境协作、人遗数据和受试者数据处理边界；
- 期望承载人数、峰值并发、文档规模、GPU/模型资源和预算；
- Dify、Harness 是否购买企业授权，LobeHub 是否获得二开商业许可。
...
- 企业内部二开和向关联公司/客户交付是否构成“distribution”；
- 是否允许改品牌、修改前后端和部署多实例；
- 是否可以使用/重建云版工作区能力；
- 商业许可的用户数、环境数、客户交付和源代码条款。

### 3.2 Dify

Dify 使用带附加条件的 Apache 2.0 修改许可证：多租户服务需要书面商业授权，使用其前端时还存在 Logo/版权修改限制。若 Youlin 面向多个客户或多个独立租户，不能默认 Community Edition 合法满足需求。
...
本轮覆盖了代码架构、认证、工作区/RBAC、智能体、任务、知识、追踪、部署、第三方平台定位和主要合规方向。未覆盖真实企业系统、用户行为、合同、基础设施容量、数据样本和现行标准作业程序（SOP）；因此产品心智模型与项目工作量仍是待验证项。
