...
> 状态：设计草案批次 1～4；不是 approved、已开发或已验收。W1 指项目实际启动周，目前没有获批日历日期。
...
- 累计 111 项 draft；其余 48 项仍 planned，尚未编写逐项详细设计；不批量复制空模板冒充完成。
...
累计 111 项为 draft，其余 48 项为 planned；全部 deliveryStatus=not_started。任何物理接口/真实 Owner/保留/SLO 未定都不隐去。catalog 的 `milestoneContexts.acContext/frContext` 为里程碑覆盖范围，逐项 `requirementRefs` 为对应分项证据范围，实际签收以详细 Spec 和原追踪矩阵为准，不宣称一项独立覆盖整组 AC。
