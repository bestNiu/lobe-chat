...
首批[可执行子集与合成测试](./executable/README.md)已加入 `RevocationCommand/RevocationState`，仅用于设计验证；[Spike A](../../08-implementation-readiness-and-spikes.md)验证实际身份与撤销链，当前未执行。
...
- 完整可执行合同、Provider capability 声明与兼容矩阵仍待补；已有禁用命令/状态的 draft Schema 与合成夹具，不覆盖 Session/身份映射的真实行为。
