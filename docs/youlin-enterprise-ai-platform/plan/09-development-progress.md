...
- 当前提前工程切片为 **5 项**，新增 M02-001-S1：真实隔离 Keycloak **14 项协议测试**、认证器 **9 项配置测试**通过；默认关闭、未接产品路由。管理员 MFA、已配置 OTP 的员工、已有 SSO 升权、JWT 过期与禁用后 refresh 均有实际协议用例。
...
| M2 统一身份 | W5 | 9/9 | 既有撤权/Reader 回归通过，新增真实 Keycloak 14 项协议和认证器9项配置测试；均未接产品 | 正式账号/HR映射与迁移、Session/PEP、服务身份、真实渠道和双端联调 |
