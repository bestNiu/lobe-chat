...
| VT-009 | M5-001/003/009 | 审批绑定参数 Hash，参数改变/过期/Actor 变化拒绝；插件 Digest 篡改、未批准安装绑定、依赖扩权升级与停用后调用拒绝；高风险 Tool 用 Mock；Pi 启用 Spike 时另测沙箱/Session/产出物，不新增生产写回 |
...
| FR-E01～03 | M05-001/003/007/008/009、M01-006 | Tool 安全、插件准入/绑定、审批参数与凭证；Pi 仅可选受控适配 |
