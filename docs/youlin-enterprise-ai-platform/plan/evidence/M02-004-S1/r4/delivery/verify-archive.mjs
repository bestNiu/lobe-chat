...
for (const item of [...manifest.sources, ...manifest.artifacts]) {
  // Root .gitignore is deliberately outside the mount allowlist. Verify its archived bytes.
  const readablePath = item.path === '.gitignore' ? `${folder}/r4/delivery/root-gitignore.snapshot` : item.path;
  const bytes = await readFile(path.join(root, readablePath));
  assert.equal(createHash('sha256').update(bytes).digest('hex'), item.sha256, item.path);
}
const deletions = await readFile(path.join(root, folder, 'r4/delivery/deleted-paths.snapshot'), 'utf8');
for (const deleted of manifest.deletedPaths) {
  assert.ok(deletions.includes(`D\t${deleted}\n`));
}
const filesByShard = [];
let total = 0;
...
console.log(`Verified ${manifest.sources.length} source hashes, ${manifest.artifacts.length} artifact hashes, archived Git template deletion, and disjoint complete ${expected.length}-file / ${total}-case shard union.`);
