...
[本轮确认记录](./11-m0-m2-confirmed-decisions.md)是 M0～M2 已确认实施输入的补充：Eric 承担责任角色；薪人薪事为人员/部门/岗位真源；Keycloak 邮箱/员工号密码登录、管理员 MFA、关闭公众注册；GitHub Actions 与 Windows/macOS；UAT 在本机 Docker 环境部署验证，其他目标环境完整部署后续推进、先准备部署文档（覆盖此前 GitLab/外部 UAT 假设）。此前“责任待认领”由此更新，但不代表全部 ADR、预算、物理合同、独立复核或31项 Spec 已获批准/验收。未确认参数继续显式保留。
...
| 资源版本/上传/安全访问 API | W11 | M06-001～003、007、009/012 的安全核心 | M7/M8 | 完整扫描隔离、不可变载荷、配额/格式限制与版本引用；W14 完成全部资源能力 |
| 知识与个人记忆 Context C1 | W15 | M07、M10-003～005 | M8 | RAG/记忆必经 Provider，不能直接拼 Prompt |
| 灯塔真实链 | W16 | M08 | M12/M13 | AC-21、当前权限/引用、最小反馈落盘与人工处理；W22 再汇入统一运营 |
...
M4 的 W8 多端交付只验当时真实可用的身份/Project/权限与客户端框架；资源、记忆、产出物和 Audience 分别随 M6/M7/M10 加入真实联测，M12 汇总完整 AC-05。早期切片通过不等于父 Spec、全量 AC 或里程碑的后续联合验收义务已关闭。
