...
- [首批可编码准备与真实 Spike 门禁](./plan/08-implementation-readiness-and-spikes.md)、[可执行合同子集及合成测试](./plan/specs/contracts/executable/README.md)
- **[当前里程碑进度与开发启动授权](./plan/09-development-progress.md)**：M02-006-S1 撤权内核和 S2 PostgreSQL 事务原型已编码、完成相应工程测试，均未接入产品。

当前为规划/设计基线，不是功能实现或企业验收完成声明。MVP PRD/交付基线为 2.4，开发计划为 1.7；先阅读统一基线再展开 Spec。计划包含 **159 项 Spec、33 项 AC、17 项待冻结决策和 26 条补充验收场景**。详细设计批次 1～6 已形成 **159 份逐项 draft**，覆盖 M0～M14，planned=0。草案覆盖不等于物理 Schema、可执行契约、测试脚本和切片细节全部就绪；完整 Spec 尚未达到正式批准/交付门。用户已批准隔离开发，固定工具链、撤权内核与 PostgreSQL 事务原型三个切片有代码及工程测试，单列执行账；仍需按 DoR 认领、核验决策、补真实接口与环境验证，不能把草案或原型当产品完成。

K01/K02/K03 已补首批 draft Schema、4 个合成示例和离线设计测试；尚未接入身份、PDP、数据库或队列，不是原 752 条待执行 Spec 用例的运行结果。
